...
动态规划  
使用dp[i][j]表示s1的前i个字符和s2的前j个字符能否构成s3的前i+j个字符  
当s1长度+s2长度!=s3长度时，显然不能构成，返回False  


<div><img src='pic/97.jpg' width=600></div>


初始化  
- dp[0][0] = True
- 初始化第一列，dp[i][0]表示s1的前i个字符能否构成s3的前i个字符，第i个字符下标为i-1  
    只要s1[i-1]=s3[i-1]，dp[i][0]为True

- 初始化第一行，dp[0][j]表示s2的前j个字符能否构成s3的前j个字符，第j个字符下标为j-1  
    只要s2[j-1]=s3[i-1]，dp[0][j]为True
...
最后返回dp[len1][len2]，判断s1,s2是否能构成s3
